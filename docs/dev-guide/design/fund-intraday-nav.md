...
4. **★ 基金名录（本轮新增，`services/fund_registry.py`）**：东财公开名录 `fund.eastmoney.com/js/fundcode_search.js`（UTF-8+BOM、`var r=[[code,拼音缩写,名称,类型,拼音全称],...]`）**覆盖全部中国注册基金**——2026-09-01 实测 27,718 条，含 01/02 等新代码段与 11x 场外段。可关注范围 = 种子 27 只 ∪ 名录 27,718 只 = **全部注册基金**。机制要点：
   ...
   - **三通道搜索**（`FundRegistry.search`，候选上限 20）：代码精确命中（**名录优先于启发式与外部搜索，命中即短路**）；中文名称子串（含「官方全称 ↔ 名录简称」容错弱匹配——如名录省略「市值加权」字样，仍命中 019118）；拼音前缀（全拼 + 缩写）与代码前缀。**查询索引（1.4 起）**：快照加载时预构建代码有序数组（bisect 代码前缀）+ 拼音首字母分桶 + 名称 ASCII token 倒排表，ASCII 按键查询只触达本字母桶与小表，不再全量 2.7 万条 `startswith`（实测拼音前缀 ~52.6ms→~1.6ms/查询）。含 CJK 仍走名称子串全扫描（子串无法索引）；**外部兜底短路（1.4 起）**：名录在载且零候选时，纯拼音（ASCII 字母 ≥6 位）与含 CJK 查询不再走 Yahoo 外部搜索——场外基金 Yahoo 本就不认识，纯浪费一次外网往返（实测零命中拼音 ~1.4s→<0.35s）；仅代码状查询（含 `.SS`/`.SZ` 等、非基金码段纯数字、≤5 位 ticker）保留外部兜底（`_external_search_worthwhile`）。名录不可用（降级）时维持原全量兜底链路；
...
- `FundNAV.vue` 详情面板：新增「估值状态」行。

**测试**：映射四分支 + 例外场景见 `frontend/tests/unit/fundStatus.spec.ts`、`watchlistStatusNote.spec.ts`、`fundNavStatusNote.spec.ts`。

...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  async function fetchDataSources() {
    const response = await apiGet<DataSourceHealthSummary>('/dashboard/data-sources')
  ...
  function updateSystemMetricFromSSE(data: Partial<DashboardSystemInfo>) {
    if (systemInfo.value) {
      ...
      const memPercent = data.memory_used_mb && data.memory_total_mb
  ...
  function updateSourceHealthFromSSE(data: DataSourceHealthDetail) {
    if (dataSources.value) {
      const index = dataSources.value.sources.findIndex((s) => s.id === data.id)
      if (index >= 0) {
        dataSources.value.sources[index] = data

        dataSources.value.healthy = dataSources.value.sources.filter((s) => s.status === 'healthy').length
        dataSources.value.degraded = dataSources.value.sources.filter((s) => s.status === 'degraded').length
        dataSources.value.down = dataSources.value.sources.filter((s) => s.status === 'down').length
      }
    }
  }
  ...
  function connectSSE() {
    ...
    sseConnection.value = new SSEConnection({
      ...
      eventHandlers: {
        [SSEEventType.SYSTEM_METRIC_UPDATE]: (data) => updateSystemMetricFromSSE(data as never),
        [SSEEventType.SOURCE_HEALTH_UPDATE]: (data) => updateSourceHealthFromSSE(data as never),
      },
    })

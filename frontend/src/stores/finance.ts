...
export const useFinanceStore = defineStore('finance', () => {
  ...
  const watchlistTop5 = computed(() => {
    const sorted = [...watchlist.value].sort((a, b) => a.display_order - b.display_order)
  ...
  async function searchSymbols(query: string, type: string = 'all') {
    ...
    const response = await apiGet<SearchResult[]>('/finance/search', { q: query, type })
  ...
  async function fetchWatchlist() {
    ...
    const quotesResponse = await apiGet<WatchlistQuote[]>('/finance/watchlist/quotes')
  ...
  async function addToWatchlist(symbol: string) {
    const response = await apiPost<WatchlistItem>('/finance/watchlist', { symbol })
  ...
  function updateQuoteFromSSE(data: { symbol: string; current_price: number; change: number; change_percent: number; volume?: number; timestamp: string }) {
  ...
  function updateMarketIndexFromSSE(data: MarketIndex) {
    const index = marketIndices.value.findIndex((i) => i.symbol === data.symbol)
    if (index >= 0) {
  ...
  function connectSSE() {
    ...
    sseConnection.value = new SSEConnection({
      ...
      eventHandlers: {
        [SSEEventType.QUOTE_UPDATE]: (data) => updateQuoteFromSSE(data as never),
        [SSEEventType.MARKET_INDEX_UPDATE]: (data) => updateMarketIndexFromSSE(data as never),
        [SSEEventType.COMMODITY_UPDATE]: (data) => updateCommodityFromSSE(data as never),
        [SSEEventType.NAV_ESTIMATE_UPDATE]: (data) => updateNAVFromSSE(data as never),
      },
    })

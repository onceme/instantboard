...
export const useSettingsStore = defineStore('settings', () => {
  ...
  async function fetchCategories(includeSources = false) {
    ...
    try {
      const response = await apiGet<Category[]>('/categories', { include_sources: includeSources })
  ...
  async function createCategory(data: Partial<Category>) {
    const response = await apiPost<Category>('/categories', data as Record<string, unknown>)
    categories.value.push(response.data)
    return response.data
  }

  async function updateCategory(id: string, data: Partial<Category>) {
    const response = await apiPut<Category>(`/categories/${id}`, data as Record<string, unknown>)
  ...
  async function createSource(data: Partial<Source>) {
    const response = await apiPost<Source>('/sources', data as Record<string, unknown>)
    sources.value.push(response.data)
    return response.data
  }

  async function updateSource(id: string, data: Partial<Source>) {
    const response = await apiPut<Source>(`/sources/${id}`, data as Record<string, unknown>)

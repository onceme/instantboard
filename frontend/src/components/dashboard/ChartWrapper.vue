...
const chartRef = ref<InstanceType<typeof Line | typeof Bar | typeof Pie | typeof Doughnut> | null>(null)
...
const themeAwareOptions = computed(() => {
  ...
  if (props.type === 'line' || props.type === 'bar') {
    ...
    base.scales.x.ticks = { ...base.scales.x.ticks, color: isDark.value ? '#94A3B8' : '#6B7280' }
    base.scales.y.ticks = { ...base.scales.y.ticks, color: isDark.value ? '#94A3B8' : '#6B7280' }
    base.scales.x.grid = { ...base.scales.x.grid, color: isDark.value ? '#334155' : '#E5E7EB' }
    base.scales.y.grid = { ...base.scales.y.grid, color: isDark.value ? '#334155' : '#E5E7EB' }
  }
  if (!base.plugins) base.plugins = {}
  base.plugins.legend = { ...base.plugins.legend, labels: { color: isDark.value ? '#E2E8F0' : '#1A1A2E' } }
  return base
})

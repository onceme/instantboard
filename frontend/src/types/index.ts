...
export interface MarketIndex {
  ...
  region: 'US' | 'CN' | 'HK' | 'JP' | 'EU_LONDON' | 'EU_FRANKFURT' | 'KR' | 'IN'
  timestamp: string
}
...
export const DOMAIN_CONFIG: Record<string, { slug: string; label: string; color: string; icon: string }> = {
  robotics: { slug: 'robotics', label: '机器人', color: '#8B5CF6', icon: '🤖' },
  ai: { slug: 'ai', label: '人工智能', color: '#3B82F6', icon: '🧠' },
  embedded: { slug: 'embedded', label: '大规模嵌入式', color: '#F59E0B', icon: '⚡' },
  space: { slug: 'space', label: '太空科技', color: '#10B981', icon: '🚀' },
}

export const SUBCATEGORY_MAP: Record<string, Array<{ slug: string; label: string }>> = {
  robotics: [
...
export type FinancePanel = 'overview' | 'watchlist' | 'search' | 'indices' | 'commodities'

// Market region groups
export const MARKET_REGION_GROUPS: Record<string, { label: string; regions: string[] }> = {
...
export const COMMODITY_GROUPS: Record<string, { label: string; category: string }> = {

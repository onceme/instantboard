...
> ⚠️ **未实现**：`db_metric_update`、`business_metric_update` — 后端 `SSEEventType` 共 10 种事件（item_update/quote_update/market_index_update/nav_estimate_update/commodity_update/alert_update/system_metric_update/source_health_update/topic_stats_update/heartbeat），不含这两种。

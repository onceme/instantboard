...
| **限流计数（滑动窗口）** | `rate:{tenant_id\|anon}:{ip}:{route_class}` | ZSET（成员=时间戳，score=毫秒时刻） | 65s（窗口 60s + 5s 余量） | **读写**：`RateLimitMiddleware`（security.md §3.3 层级 2）每请求经 `redis_sliding_window_count` 单条非事务 pipeline 写读——`ZADD` 记录本次时间戳（成员带 uuid 后缀防同毫秒塌缩）→ `ZREMRANGEBYSCORE key 0 (now-60s)` 裁剪滑出窗口的成员 → `ZCARD` 判定（> 阈值+burst 即 429）→ `EXPIRE` 65s 回收空闲桶。tenant 尽力不验签解码（无/非法 → `anon`），ip 走 `get_client_ip` 右起解析，route_class ∈ auth/search/default；Redis 异常 → 放行（fail-open） |
